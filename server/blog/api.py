...
def newBlog(request):
    ...
    blog_felids = {
        ...
        "description": request.data.get("headline"),
...
def updateBlog(request, slug):
    ...
    if blog_serializer_form.is_valid():
        ...
        serialized_data = {**BlogSerializer(blog).data, "tags": tags}
        return Response(serialized_data, status=status.HTTP_201_CREATED)
    else:
        return Response(blog_serializer_form.errors, status=status.HTTP_400_BAD_REQUEST)
...
def removeThumbnail(request):
    ...
    return Response(
        {"success": "thumbnail is deleted"}, status=status.HTTP_202_ACCEPTED
    )
...
def getBlogCollections(request):
    recent_blogs = get_blog_list_data(
        request,
        Blog.objects.order_by("-published").filter(status="published").distinct()[:5],
    )
    pin_blogs = get_blog_list_data(
        request,
        Blog.objects.order_by("-published")
        .filter(status="published", pin_to_top=True)
        .distinct(),
    )
    serialized_data = {
        "pin_blogs": pin_blogs,
...
def getRecentBlogs(request):
    serialized_data = get_blog_list_data(
        request,
        Blog.objects.order_by("-published").filter(status="published").distinct()[:5],
    )
    return Response(serialized_data, status=status.HTTP_200_OK)
...
def getLastBlogs(request):
    blog = Blog.objects.order_by("-published").filter(status="published").first()
...
def getBlogDetails(request, slug):
    ...
    if blog.category:
        blog_data["category"] = blog.category.name
    serialized_data = {**blog_data, "comments": comments}
    return Response(serialized_data, status=status.HTTP_200_OK)
...
def getRelatedBlogs(request, slug):
    ...
    blogs = (
        Blog.objects.order_by("-published")
        .filter(status="published", category__name__icontains=blog.category)
        .exclude(id=blog.id)
    )
    serialized_data = get_blog_list_data(request, blogs)
    return Response(serialized_data, status=status.HTTP_200_OK)
...
def getBlogFilter(request):
    ...
    categories = CategorySerializer(
        Category.objects.filter(blog__status="published").distinct(), many=True
    ).data
    tags = TagSerializer(
        Tag.objects.filter(blog__status="published").distinct(), many=True
    ).data
    serialized_data = {
        "archives": archives,
...
def getAllCategory(request):
    categories = CategorySerializer(
        Category.objects.filter(blog__status="published"), many=True
    ).data
...
def addBlogComment(request, slug):
    ...
    if comment_serializer_form.is_valid():
        ...
        blog.save()
        return Response(
            BlogCommentSerializer(comment).data, status=status.HTTP_201_CREATED
        )
